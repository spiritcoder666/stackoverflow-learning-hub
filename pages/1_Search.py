# =============================================================================
# Stack Overflow Learning Hub - V6 (Recommendations)
# =============================================================================
...
def find_similar_questions(query, top_k=5, user_tags=None):
    ...
    combined_df = pd.concat([exact_match_df, semantic_results_df]).drop_duplicates(
        subset=["Id"], keep="first"
    )
    final_results = combined_df
    final_results["PersonalizationScore"] = final_results["CleanTags"].apply(
        lambda tags: 1 if user_tags and any(tag in tags for tag in user_tags) else 0
    )
    final_results["CombinedScore"] = (0.9 * final_results["Similarity"]) + (
        0.1 * final_results["PersonalizationScore"]
    )
...
st.markdown(
    "Describe your problem to find the best existing questions and their top-rated answers."
)
query = st.text_input(
    "**Enter your question or problem description**",
    placeholder="e.g., how to sort a python dictionary by value",
    key="search_query",
)
...
if query:
    ...
    # --- SAVE SEARCH HISTORY ---
    # We save the ID of the top search result to represent the query
    ...
    if not recommendations.empty:
        for _, row in recommendations.iterrows():
            ...
            with st.container(border=True):
                if row["PersonalizationScore"] > 0:
                    st.markdown("⭐ **Personalized for you!**")
                st.markdown(
                    f"#### [{row['Title']}](https://stackoverflow.com/q/{question_id})"
                )
                col1, col2, col3 = st.columns([3, 1, 1])
                with col1:
                    st.caption(f"Tags: `{row['CleanTags']}`")
                with col2:
                    st.metric(label="Relevance", value=f"{row['CombinedScore']:.2f}")
                with col3:
